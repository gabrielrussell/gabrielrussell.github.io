# Blog Rules

## Posts
- Never write prose or descriptive content in blog posts. Gabriel writes all post content.
- When reviewing a post, only flag objective errors: spelling, grammar, broken links, incorrect dates. Do not suggest changes for clarity, style, or tone.
- When creating a post from URLs, set the title and include links/metadata gleaned from the source material, but do not write descriptions, summaries, or commentary. Use lorem ipsum as placeholder body text for Gabriel to replace.
